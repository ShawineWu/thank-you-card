...
export interface ValueColorConfig {
  ...
  // CSS values for inline styles (Fluent UI)
...
// Helper function to get inline style object for Fluent UI components
export function getValueInlineStyle(valueName: string): React.CSSProperties {
  const color = getValueColor(valueName);
  return {
    backgroundColor: color.bgCss,
    color: color.textCss,
    borderColor: color.borderCss,
  };
}
