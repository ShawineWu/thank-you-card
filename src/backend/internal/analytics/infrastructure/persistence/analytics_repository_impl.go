...
import (
	...
	"github.com/castlery/thank-you-card/internal/analytics/domain/repositories"
	"gorm.io/gorm"
)
...
func NewGormAnalyticsRepository(db *gorm.DB) repositories.AnalyticsRepository {
...
func (r *GormAnalyticsRepository) GetTopValues(limit int) ([]repositories.ValueUsage, error) {
	var results []repositories.ValueUsage
	query := `
		SELECT 
			cv.id as value_id,
...
func (r *GormAnalyticsRepository) GetTopRecognizers(limit int) ([]repositories.TopRecognizer, error) {
	var results []repositories.TopRecognizer
	query := `
		SELECT 
...
func (r *GormAnalyticsRepository) GetTeamAnalytics() ([]repositories.TeamStats, error) {
	...
	var results []repositories.TeamStats
...
func (r *GormAnalyticsRepository) GetValueDistribution() ([]repositories.ValueUsage, error) {
	var results []repositories.ValueUsage
	query := `
		SELECT 
			cv.id as value_id,
...
func (r *GormAnalyticsRepository) GetCardsForExport(startDate, endDate time.Time) ([]repositories.ExportCard, error) {
	var cards []struct {
		ID                string
		SenderID          string
	...
	exportCards := make([]repositories.ExportCard, len(cards))
	for i, card := range cards {
		...
		exportCards[i] = repositories.ExportCard{
			ID:                card.ID,
